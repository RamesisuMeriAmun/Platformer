...
class LevelEditor:
    ...
    def save_map(self):
        data = {"grid": self.tilemap, "offgrid": self.offgrid_tiles, "rooms": self.rooms_list}
    ...
    def run(self):
        ...
        while run:
            ...
            for room in self.rooms_list:
                ...
                # Raum-Rechteck
                rect_screen = pygame.Rect((r[0] - self.scroll[0]) * self.zoom, (r[1] - self.scroll[1]) * self.zoom,
                                          r[2] * self.zoom, r[3] * self.zoom)
                pygame.draw.rect(self.window, color, rect_screen, 2)

                # Spawn-Punkt
                if "spawn" in room:
                    sp = room["spawn"]
                    spawn_rect = pygame.Rect((sp[0] - self.scroll[0]) * self.zoom, (sp[1] - self.scroll[1]) * self.zoom,
                                             10 * self.zoom, 10 * self.zoom)
                    pygame.draw.rect(self.window, (255, 0, 255), spawn_rect)

            if self.room_mode and self.room_start_pos:
                curr_rect = [min(self.room_start_pos[0], world_x), min(self.room_start_pos[1], world_y),
                             abs(self.room_start_pos[0] - world_x), abs(self.room_start_pos[1] - world_y)]
                pygame.draw.rect(self.window, (255, 255, 255),
                                 ((curr_rect[0] - self.scroll[0]) * self.zoom,
                                  (curr_rect[1] - self.scroll[1]) * self.zoom,
                                  curr_rect[2] * self.zoom, curr_rect[3] * self.zoom), 1)
            ...
            # Zeile 2
            info_line2 = self.font.render(f"Zoom: {int(self.zoom * 100)}% | [O] Save | [L] Load | [G] Grid",
                                          True, (255, 255, 255))
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    if event.button == 1 and self.room_start_pos:
                        ...
                        self.rooms_list.append({
                            ...
                            "spawn": [new_x + BLOCK_SIZE // 2, new_y + BLOCK_SIZE // 2]
                        })
